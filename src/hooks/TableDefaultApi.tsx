...
const TableDefaultApi: React.FC<TableDefaultProps> = ({
  columns,
  data = [],
  actions = [],
  fixedHeader = false,
  ...
  const handleSort = (key: string) => {
    ...
    // Se temos uma função de callback para ordenação na API, usamos ela
    if (onSortChange) {
      onSortChange(key, direction)
    } else {
      // Caso contrário, faz a ordenação localmente
      setCurrentPage(1)
    }
  }
  ...
  const toggleSelectAll = () => {
    if (isAllSelected || selectedRows.length === data.length) {
      setSelectedRows([])
  ...
  useEffect(() => {
    if (onSelectionChange) {
      onSelectionChange(selectedRows.map(index => data[index]))
    }
            ...
            <tr>
              {selectable && (
                <th className={`${headerClasses} px-4 py-3 w-10 sticky top-0 z-10 bg-gray-50 dark:bg-gray-800/50 dark:backdrop-blur`}>
                  <input
                    type="checkbox"
                    checked={isAllSelected || (selectedRows.length === data.length && data.length > 0)}
            ...
            {actions
              .filter(action => {
                if (typeof action.hidden === 'function') {
                  return !action.hidden(data[openActionRow])
                }
              ...
              .map((action, index) => (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    action.handleClick(data[openActionRow])
                    setOpenActionRow(null)
                    setActionMenuPosition(null)
                  }}
...
export default TableDefaultApi